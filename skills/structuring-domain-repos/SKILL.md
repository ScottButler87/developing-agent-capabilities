...
- **CI on every PR**: validate frontmatter, reject duplicate skill names, post a per-subdomain skill count in the job summary, and run `reuse lint` — see [licensing-new-files](../licensing-new-files/SKILL.md) for what a failure means and how to fix it. If the repo has any skill on a non-default license (see above), also run `sync-skill-license.py --all --check` so a skill's frontmatter can't merge out of sync with its own `REUSE.toml` entry. Add this on the first PR accepted from anyone, not before.
