...
- **CONTRIBUTING.md**: five numbered steps (create the skill directory, fill frontmatter, required body sections, optional supporting files, PR title convention `Add skill: <name>`), a quality checklist, the closed subdomain/category list — a workable starting shape, adapt as needed. Point contributors at [authoring-domain-skills](../authoring-domain-skills/SKILL.md) for the actual writing conventions rather than duplicating them here. Don't publish a review-time commitment (e.g. "reviewed within 48 hours") a solo or small maintainer can't reliably hold — state responsiveness as an aspiration or omit it; a missed public commitment costs more contributor trust than none.
...
- **GitHub issue forms** (`.yml`, not `.md`) with closed dropdowns for category/type and `blank_issues_enabled: false`. Front-loads the taxonomy decision onto the contributor at filing time.
- **CI on every PR**: validate frontmatter, reject duplicate skill names, post a per-subdomain skill count in the job summary, and run `reuse lint` — see [licensing-new-files](../licensing-new-files/SKILL.md) for what a failure means and how to fix it. Add this on the first PR accepted from anyone, not before.
